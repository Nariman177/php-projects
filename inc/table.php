...
<body>
  ...
  <div id="nav">
    ...
    if(isset($_GET['cols']) && isset($_GET['rows'])){
           $cols = (int) $_GET['cols'];
           $rows = (int) $_GET['rows'];
           echo "<table border = '1' width='300' style = 'background-color: yellow; '>";
             for ($tr = 1; $tr <= $rows; $tr++) { // минимальная таблица умножения - это 1 х 1
                     echo "<tr style = 'background-color: yellow '>"; // рисует строку (внутри <tr> находится <td> - нужен очередной цикл)
                     for ($td = 1; $td <= $cols; $td++) {
                             echo "<td>" . ($tr * $td) . "</td>"; // $tr * $td - для вывода значений (цифр) внутри таблицы
                         }
                     echo "</tr>";
